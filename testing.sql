...
REVOKE SELECT, INSERT, UPDATE, DELETE ON users FROM wishlist_user;
REVOKE SELECT, INSERT, UPDATE, DELETE ON wishlists FROM wishlist_user;
REVOKE SELECT, INSERT, UPDATE, DELETE ON wishlist_items FROM wishlist_user;
...
GRANT SELECT, INSERT, UPDATE ON users TO wishlist_user;
GRANT SELECT, INSERT, UPDATE ON wishlists TO wishlist_user;
GRANT SELECT, INSERT, UPDATE ON wishlist_items TO wishlist_user;

-- IDK why sequences need its own permission
GRANT USAGE, SELECT ON SEQUENCE wishlists_id_seq TO wishlist_user;
GRANT USAGE, SELECT ON SEQUENCE wishlist_items_id_seq TO wishlist_user;

-- users table is always insertable (create new users)
CREATE POLICY users_insert_rls_policy ON public.users
	FOR INSERT
	TO wishlist_user
	WITH CHECK (true);

-- Getting data is only allowed to the own user
CREATE POLICY users_select_rls_policy ON public.users
	FOR SELECT
	TO wishlist_user
  USING (id = current_setting('app.current_user_id')::int4);

-- Same thing for writting data
CREATE POLICY users_update_rls_policy ON public.users
	FOR UPDATE
	TO wishlist_user
  WITH CHECK (id = current_setting('app.current_user_id')::int4);
...
CREATE POLICY wishlists_rls_policy ON public.wishlists
  FOR ALL
	TO wishlist_user
	USING (user_id = current_setting('app.current_user_id')::int4)
	WITH CHECK (user_id = current_setting('app.current_user_id')::int4);

-- Exactly same thing for wishlist_items
CREATE POLICY wishlist_items_rls_policy ON public.wishlist_items
	FOR ALL
	TO wishlist_user
	USING (wishlist_id IN (SELECT id FROM public.wishlists WHERE user_id = current_setting('app.current_user_id')::int4))
	WITH CHECK (wishlist_id IN (SELECT id FROM public.wishlists WHERE user_id = current_setting('app.current_user_id')::int4));
